  <template>
    ...
    <div class="pagination">
      <button :disabled="page === 0" @click="prevPage">⬅️ Trước</button>
      <span>Trang {{ page + 1 }} / {{ totalPages }}</span>
      <button :disabled="page >= totalPages - 1" @click="nextPage">Tiếp ➡️</button>
    </div>
  </template>
  ...
  // ==== State phân trang ====
  // const pageCache = ref<Record<number, Book[]>>({});
  // const page = ref(0);
  // const size = ref(5);
  // const books = ref<Book[]>([]);
  // const totalPages = ref(0);
  //
  // const { result, refetch } = useQuery(SEARCH_BOOK_PAGE_NATIVE, {
  //   page: page.value,
  //   size: size.value,
  // });
  const page = ref(0);
  ...
  // watch(result, (data) => {
  //   if (data?.getBooksByPage) {
  //     pageCache.value[page.value] = data.getBooksByPage.content; // ✅ lưu cache theo page
  //     books.value = pageCache.value[page.value];
  //     totalPages.value = data.getBooksByPage.totalPages;
  //   }
  // });
  //
  // function nextPage() {
  ...
  //     page.value++;
  //     if (pageCache.value[page.value]) {
  //       // ✅ Nếu có cache, lấy ra luôn
  //       books.value = pageCache.value[page.value];
  //     } else {
  //       // ❌ Nếu chưa có thì gọi server
  ...
  //   }
  // }
  const books = computed<Book[]>(() => result.value?.getBooksByPage?.content || []);
  const totalPages = computed<number>(() => result.value?.getBooksByPage?.totalPages || 0);
